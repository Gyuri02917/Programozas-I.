...
int ar, db;
    ...
    cout.setf(ios::left); // 5. feladat eredmények kiíratása "táblázatosan"
    for (i = 0; i < aruszam; i++)
    {
        cout << setw(20) << bolt[i].arunev << bolt[i].ar << "Ft" << "\t"
        << bolt[i].db << "db" << endl;
        ertek = ertek + bolt[i].ar * bolt[i].db; //6. egész árukészlet értéke
        atlagar = +bolt[i].ar; // 7. feladat - átlagár kiszámítása
        if (bolt[i].db > 5) otneltobb++;//8. hány áruból van több mint öt
        if (bolt[i].ar > bolt[legdragabb].ar) legdragabb = i; //9. feladat- Írja ki a legdrágább áru nevét és árát!
    }
    cout << "\nAz arukeszlet ossz ereteke: " << ertek << "Ft" << endl;
    cout << "\nAz aruk atlagara: " << atlagar/aruszam << "Ft" << endl;
    cout << endl << otneltobb << " arubol van otnel tobb a keszleten."<< endl;
    cout << "\nA legdragabb aru adatai: \n" << setw(20) <<bolt[legdragabb].arunev << bolt[legdragabb].ar << "Ft" << "\t" << bolt[legdragabb].arunev << "\t" << bolt[legdragabb].ar << endl;
